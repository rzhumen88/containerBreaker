...
class Container:
    ...
    def getTable(self):
        ...
        with open(self.std, 'rb') as self.index:
            ...
            for self.i in range(self.total):
                ...
                self.sfxname = 'sfx'+str(self.i).zfill(8)
    ...
    def getFile(self, fname, offset, size, csize, args):
        with open(self.sfx, 'rb') as self.f:
            ...
            self.wav = b"RIFF" + struct.pack('I', size+36) + b"WAVEfmt " + struct.pack('IHHIIHH', 16, 1, 1, self.samples, self.samples*2, 2, 16) + b'data'# + struct.pack('I', size)
